class Api::V1::ReviewsController < Api::ApiBase
  ...
  def create
    ...
    if reviews.present?
      status = reviews.first.update_attributes(name: params[:name], rating: params[:rating], text: params[:text])
    else
      review = Review.new(name: params[:name], rating: params[:rating], text: params[:text], product_id: params[:product_id], user_id: current_user.id, email: current_user.email)
      status = review.save
    end
