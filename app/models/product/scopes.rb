class Product < Base
  ...
  add_search_scope :with_option do |option|
    ...
    conditions = case option
                   when String then
                     {"#{option_types}.name" => option}
                   when OptionType then
                     {"#{option_types}.id" => option.id}
                   else
                     {"#{option_types}.id" => option.to_i}
                 end
  ...
  add_search_scope :with_option_value do |option, value|
    ...
    option_type_id = case option
                       when String then
                         OptionType.find_by(name: option) || option.to_i
                       when OptionType then
                         option.id
                       else
                         option.to_i
