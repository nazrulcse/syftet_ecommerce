class Promotion < Base
  ...
  has_many :orders, through: :order_promotions, class_name: 'Order'
  ...
  scope :applied, lambda {
                  joins(<<-SQL).uniq
        INNER JOIN order_promotions
        ON order_promotions.promotion_id = #{table_name}.id
                  SQL
                }
  ...
  def products
    rules.where(type: "Promotion::Rules::Product").map(&:products).flatten.uniq
  end
