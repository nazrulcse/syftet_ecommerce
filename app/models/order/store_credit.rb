  ...
  module StoreCredit
    def add_store_credit_payments
      ...
      if user && user.store_credits.any?
        payment_method = PaymentMethod::StoreCredit.available.first
    ...
    def display_total_applicable_store_credit
      Money.new(-total_applicable_store_credit, currency: currency)
    end

    def display_total_applied_store_credit
      Money.new(-total_applied_store_credit, currency: currency)
    end

    def display_order_total_after_store_credit
      Money.new(order_total_after_store_credit, currency: currency)
    end

    def display_total_available_store_credit
      Money.new(total_available_store_credit, currency: currency)
    end

    def display_store_credit_remaining_after_capture
      Money.new(total_available_store_credit - total_applicable_store_credit, currency: currency)
    end
