...
handle_delete = (e, data) ->
  ...
  response = confirm(translations.are_you_sure_delete)
  if response
    $.ajax
      type: "POST",
      dataType: "json",
...
root.setup_taxonomy_tree = (taxonomy_id) ->
  ...
  if taxonomy_id != undefined
    ...
    root.base_url = Syftet.url(taxonomy_taxons_path)

    $.ajax
      url: Syftet.url(base_url.path().replace("/taxons", "/jstree")).toString(),
      data:
        token: Syftet.api_key
      success: (taxonomy) ->
        last_rollback = null
        ...
        conf =
          json_data:
            ...
            ajax:
              url: (e) ->
                Syftet.url(base_url.path() + '/' + e.prop('id') + '/jstree' + '?token=' + Syftet.api_key).toString()
          themes:
            theme: "spree",
            url: Syftet.url(jstree_theme_path)
          strings:
            new_node: new_taxon,
